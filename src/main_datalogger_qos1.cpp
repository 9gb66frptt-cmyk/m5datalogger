...
#include <MQTT.h>                  // 256dpi/arduino-mqtt (supporte QoS 1)
...
MQTTClient mqtt(4096);  // arduino-mqtt — buffer interne 4KB pour cycles
...
void mqttCallback(String& topic, String& payload) {
    Serial.printf("[CMD] recu sur '%s' (%d octets): %s\n",
...
// Une seule tentative de connexion ; appelée depuis setup() puis reprise
// par le loop() si déconnecté (avec rate-limit MQTT_RECONNECT_INTERVAL).
bool tryConnectMqtt() {
    Serial.printf("[MQTT] connect host=%s:%d clientId=%s userLen=%d passLen=%d\n",
                  cfgMqttHost.c_str(), cfgMqttPort, deviceId.c_str(),
                  cfgMqttUser.length(), cfgMqttPass.length());
    ...
    if (!mqtt.connect(deviceId.c_str(), cfgMqttUser.c_str(), cfgMqttPass.c_str())) {
        Serial.printf("[MQTT] connect fail returnCode=%d lastError=%d\n",
                      mqtt.returnCode(), mqtt.lastError());
        return false;
    }
    Serial.println("[MQTT] connect OK");

    mqtt.publish(TOPIC_STATUS, onlineMessage.c_str(), true /*retained*/, 1 /*qos*/);
    mqtt.subscribe(TOPIC_COMMAND.c_str(), 1 /*qos*/);
    Serial.printf("[MQTT] subscribed to %s (QoS 1)\n", TOPIC_COMMAND.c_str());

    // Heartbeat immédiat pour annoncer l'identité (nom autoclave, série, cabinet).
    sendHeartbeat();
...
void connectMqtt() {
    ...
    mqtt.begin(cfgMqttHost.c_str(), cfgMqttPort, net);
    mqtt.onMessage(mqttCallback);

    if (tryConnectMqtt()) {
        M5.Display.setTextColor(TFT_GREEN);
        M5.Display.println("MQTT OK (QoS 1)");
    } else {
        M5.Display.setTextColor(TFT_RED);
        M5.Display.printf("MQTT KO rc=%d err=%d\n",
                          mqtt.returnCode(), mqtt.lastError());
    }
}
...
bool flushOnePending() {
    ...
    Serial.printf("[BUF] flush %s (%u bytes, QoS 1)...\n", path.c_str(),
                  (unsigned)payload.length());
    // QoS 1 : si la connexion drop avant le PUBACK, arduino-mqtt retourne
    // false et on garde le fichier pour la prochaine tentative.
    if (mqtt.publish(TOPIC_CYCLE, payload.c_str(), false /*retained*/, 1 /*qos*/)) {
        LittleFS.remove(path);
        totalCyclesSent++;
...
void sendCycleData(const String& rawData) {
    ...
    Serial.printf("[MQTT] Envoi cycle (%d bytes, QoS 1)...\n", payload.length());

    if (mqtt.connected() &&
        mqtt.publish(TOPIC_CYCLE, payload.c_str(), false /*retained*/, 1 /*qos*/)) {
        totalCyclesSent++;
        String ts = localNow();
        if (ts.length() > 0) lastCycleDisplay = ts;
        Serial.println("[MQTT] Cycle publie OK (QoS 1)");
        showStatus("CYCLE ENVOYE", TFT_GREEN, "QoS 1 - ACK broker", TFT_WHITE);
        return;
    }
...
void loop()
{
    ...
    // arduino-mqtt nécessite mqtt.loop() régulier (parsing pkts entrants).
    mqtt.loop();
    if (!mqtt.connected() &&
